...
export class Heap<X> {
  ...
  incRc(key: Key) {
    let count = this.rc.get(key.base);
    if (count !== undefined) {
      this.rc.set(key.base, count + 1);
    }
  }
...
function incRc(val: Value, state: State) {
  if (typeof val === "object" && !(val instanceof BigInt)) {
    state.heap.incRc(val.loc);
  }
}

function decRc(val: Value | undefined, state: State) {
  if (typeof val === "object" && !(val instanceof BigInt)) {
    state.heap.decRc(val.loc);
  }
}

/**
 * Update reference counts that may have changed after an assignment, where the
 * assignment overwrites `oldVal` with `newVal`.
 */
function updateRc(newVal: Value, oldVal: Value | undefined, state: State) {
  incRc(newVal, state);
  decRc(oldVal, state);
}
...
function evalCall(instr: bril.Operation, state: State): Action {
  ...
  if (!("dest" in instr)) { // `instr` is an `EffectOperation`.
    ...
    updateRc(retVal, state.env.get(instr.dest), state);
    state.env.set(instr.dest, retVal);
  }

  for (let val of newEnv.values()) {
    decRc(val, state);
  }
...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  switch (instr.op) {
    ...
    case "id": {
      ...
      updateRc(val, state.env.get(instr.dest), state);
    ...
    case "alloc": {
      ...
      updateRc(ptr, state.env.get(instr.dest), state);
    ...
    case "store": {
      ...
      updateRc(val, state.heap.tryRead(target.loc), state);
      state.heap.write(
        target.loc,
        val,
      );
    ...
    case "phi": {
      ...
      if (idx === -1) {
        ...
        decRc(state.env.get(instr.dest), state);
