...
class Trace {
  ...
  constructor(func: string) {
    this.func = func;
    this.trace = [{ op: "speculate" }];
    this.state = TraceState.NotStarted;
    this.tmp = 0;
  }

  start(func: string) {
    switch (this.state) {
      case TraceState.NotStarted:
        if (func == this.func) {
          this.state = TraceState.Ongoing;
        }
        break;
      case TraceState.Ongoing:
        this.abort();
        break;
    }
  }

  push(instr: bril.Instruction) {
    if (this.state == TraceState.Ongoing) {
      this.trace.push(instr);
    }
  }

  guard(instr: bril.Operation, val: boolean) {
    if (this.state == TraceState.Ongoing) {
  ...
  commit(ret: bril.Instruction | null = null) {
    if (this.state == TraceState.Ongoing) {
      this.trace.push({ op: "commit" });

      if (ret !== null) {
        this.trace.push(ret);
      }

      this.state = TraceState.Finished;
    }
  }

  abort() {
    if (this.state == TraceState.Ongoing) {
      throw error("aborting tracing");
    }
  }
...
function evalProg(prog: bril.Program) {
  ...
  // More silly parsing to find the `-t` flag.
  let trace_func: string | null = null;
  const t_idx = args.indexOf("-t");
  if (t_idx > -1) {
    trace_func = args[t_idx + 1];
    args.splice(t_idx, 2);
  }
  ...
  let state: State = {
    ...
    curlabel: null,
    specparent: null,
    trace: trace_func !== null ? new Trace(trace_func) : null,
  };
  evalFunc(main, state);

  if (state.trace !== null) {
    console.log(state.trace.json());
  }
